...
def parase_args():
    ...
    parser.add_argument('--learningrate', type=float, default=0.0001,
                        help='Learning Rate.')
    parser.add_argument('--trainchoice', nargs='?', default="No",
...
def main():
    ...
    with tf.device('/gpu:0'):
        ...
        with open('EncoreParameter.pickle', 'rb') as f:  # Python 3: open(..., 'wb')
            para_data=pickle.load(f)
            sess.run(encore.Em.assign(para_data[0]))
            sess.run(encore.Et.assign(para_data[1]))
            sess.run(encore.W.assign(para_data[2]))
            sess.run(encore.E.assign(para_data[3]))
            sess.run(encore.b1.assign(para_data[4]))
            sess.run(encore.c.assign(para_data[5]))
            ...
            for key in Testkeys:
                ...
                for it in checklist:
                    ...
                    try:
                        ...
                        print("test1 thresh is", test_thresh,"test1 acc is",cal_acc)
                        result_acc.append(cal_acc)
                        ichecklist = ichecklist + 1
                        # print("runtest")
                    except:
                ...
                for it in Qchecklist:
                    ...
                    try:
                        ...
                        print("test2 thresh is", test_thresh,"test2 acc is",cal_acc)
                        result_acc.append(cal_acc)
                        ical = ical + 1
                        # print("runText2")
                    except:
        ...
        with open('EncoreParameter.pickle', 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump(
                [sess.run(encore.Em), sess.run(encore.Et), sess.run(encore.W), sess.run(encore.E), sess.run(encore.b1),
                 sess.run(encore.c)], f)
        '''
